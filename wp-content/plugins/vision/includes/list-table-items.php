...
class Vision_List_Table_Items extends WP_List_Table {
	...
	function column_title($item) {
		...
		if(current_user_can('administrator') || get_current_user_id()==$item['author']) {
			$actions = array(
				'edit'      => sprintf('<a href="?page=%s&id=%s">%s</a>', VISION_PLUGIN_NAME . '_item', $item['id'], __('Edit', VISION_PLUGIN_NAME)),
				'copy'      => sprintf('<a href="?page=%s&action=%s&id=%s&_wpnonce=%s">%s</a>', $page, 'duplicate', $item['id'], wp_create_nonce(VISION_PLUGIN_NAME), __('Duplicate', VISION_PLUGIN_NAME)),
				'delete'    => sprintf('<a href="?page=%s&action=%s&id=%s&_wpnonce=%s">%s</a>', $page, 'delete', $item['id'], wp_create_nonce(VISION_PLUGIN_NAME), __('Delete', VISION_PLUGIN_NAME)),
			);
			
			return sprintf('<a href="?page=%1$s&id=%2$s" class="row-title">%3$s</a> %4$s',
				/*%1$s*/ VISION_PLUGIN_NAME . '_item',
				/*%2$s*/ $item['id'],
				/*%3$s*/ $item['title'],
	...
	function column_shortcode($item) {
		return sprintf('<code>[vision id="%1$s"]</code>',
			/*%1$s*/ $item['id']
		);
	}
	...
	function get_columns() {
		$columns = array(
			...
			'title'     => __('Title', VISION_PLUGIN_NAME),
			'active'    => __('Active', VISION_PLUGIN_NAME),
			'shortcode' => __('Shortcode', VISION_PLUGIN_NAME),
			'author'    => __('Author', VISION_PLUGIN_NAME),
			'date'      => __('Date', VISION_PLUGIN_NAME),
			'modified'  => __('Modified', VISION_PLUGIN_NAME)
		);
		return $columns;
	}
