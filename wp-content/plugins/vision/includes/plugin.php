...
class Vision_Builder {
	...
	function run() {
		...
		define('VISION_PLUGIN_UPLOAD_URL', $upload_dir['baseurl'] . '/' . VISION_PLUGIN_NAME . '/');
	...
	function add_rewrite_rule() {
		add_rewrite_tag('%vision%', '([^&]+)');
		add_rewrite_rule('^vision/([^/]*)/?', 'index.php?vision=$matches[1]', 'top');
	}
	
	function rewrite_catch() {
		global $wp_query;
		
		if(array_key_exists('vision', $wp_query->query_vars)) {
	...
	function getMainCss($itemData, $itemId) {
		...
		foreach($itemData->layers as $layerKey => $layer) {
			...
			$layerId++;
			$layerSelector = '.vision-map-' . $itemId . ' .vision-lr-' . $layerId;
	...
	function shortcode($atts) {
		...
		if(!$id) {
			return '<p>' . __('Error: invalid vision shortcode attributes', VISION_PLUGIN_NAME) . '</p>';
		}
		...
		if($item) {
			...
			if(sizeof($this->shortcodes) == 1) {
				$plugin_url = plugin_dir_url(dirname(__FILE__));
				wp_enqueue_script(VISION_PLUGIN_NAME . '_loader', $plugin_url . 'assets/js/loader.min.js', array('jquery'), VISION_PLUGIN_VERSION, true);
				...
				wp_localize_script(VISION_PLUGIN_NAME . '_loader', VISION_PLUGIN_NAME . '_globals', $globals);
			}
			...
			return '<p>' . __('Error: invalid vision database record', VISION_PLUGIN_NAME) . '</p>';
		}
	}
	...
	function admin_notices() {
		...
		if(!($page===VISION_PLUGIN_NAME || $page===VISION_PLUGIN_NAME . '_settings')) {
			return;
		}
		...
		if(!file_exists(VISION_PLUGIN_UPLOAD_DIR)) {
			...
			echo '<p>' . sprintf(__('The "%s" directory could not be created', VISION_PLUGIN_NAME), '<b>' . VISION_PLUGIN_NAME . '</b>') . '</p>';
			echo '<p>' . __('Please run the following commands in order to make the directory', VISION_PLUGIN_NAME) . '<br>';
		...
		if(!wp_is_writable(VISION_PLUGIN_UPLOAD_DIR)) {
			echo '<div class="notice notice-error is-dismissible">';
			echo '<p>' . sprintf(__('The "%s" directory is not writable, therefore the css and js files cannot be saved.', VISION_PLUGIN_NAME), '<b>' . VISION_PLUGIN_NAME . '</b>') . '</p>';
			echo '<p>' . __('Please run the following commands in order to make the directory', VISION_PLUGIN_NAME) . '<br>';
			echo '<b>chmod 777 ' . VISION_PLUGIN_UPLOAD_DIR . '</b></p>';
			echo '</div>';
			return;
	...
	function admin_menu() {
		...
		add_menu_page(__('Vision', VISION_PLUGIN_NAME), __('Vision', VISION_PLUGIN_NAME), 'manage_options', VISION_PLUGIN_NAME, array( $this, 'admin_menu_page_items' ), 'dashicons-format-image');
		add_submenu_page(VISION_PLUGIN_NAME, __('Vision', VISION_PLUGIN_NAME), __('All Items', VISION_PLUGIN_NAME), 'manage_options', VISION_PLUGIN_NAME, array( $this, 'admin_menu_page_items' ));
		add_submenu_page(VISION_PLUGIN_NAME, __('Vision', VISION_PLUGIN_NAME), __('Add New', VISION_PLUGIN_NAME), 'manage_options', VISION_PLUGIN_NAME . '_item', array( $this, 'admin_menu_page_item' ));
		add_submenu_page(VISION_PLUGIN_NAME, __('Vision', VISION_PLUGIN_NAME), __('Settings', VISION_PLUGIN_NAME), 'manage_options', VISION_PLUGIN_NAME . '_settings', array( $this, 'admin_menu_page_settings' ));
	}
	...
	function admin_menu_page_items() {
		...
		if($page===VISION_PLUGIN_NAME) {
			...
			wp_enqueue_style(VISION_PLUGIN_NAME . '_vision_font_icons_css', $plugin_url . 'assets/css/vision-font-icons.min.css', array(), VISION_PLUGIN_VERSION, 'all' );
			...
			$globals = array(
				'plan' => VISION_PLUGIN_PLAN,
				'msg_pro_title' => __('Available only in Pro version', VISION_PLUGIN_NAME),
				'upload_url' => $upload_dir['baseurl'],
				'ajax_url' => admin_url( 'admin-ajax.php' ),
				'ajax_nonce' => wp_create_nonce( VISION_PLUGIN_NAME . '_ajax' ),
				'ajax_msg_error' => __('Uncaught Error', VISION_PLUGIN_NAME) //Look at the console (F12 or Ctrl+Shift+I, Console tab) for more information
			);
			...
			if($action && $nonce && wp_verify_nonce($nonce, VISION_PLUGIN_NAME)) {
				global $wpdb;
				$table = $wpdb->prefix . VISION_PLUGIN_NAME;
				...
				if($action == 'duplicate') {
					...
					if($item && (current_user_can('administrator') || get_current_user_id()==$item->author) ) {
						$itemData = unserialize($item->data);
						$itemData->title = __('[Duplicate] ', VISION_PLUGIN_NAME) . $itemData->title;
						...
						if($result && wp_is_writable(VISION_PLUGIN_UPLOAD_DIR)) {
							$file_json = 'config.json';
							$file_main_css = 'main.css';
							$file_custom_css = 'custom.css';
							$file_root_path = VISION_PLUGIN_UPLOAD_DIR . '/' . $wpdb->insert_id . '/';
				...
				if($action=='delete') {
					...
					if($item && (current_user_can('administrator') || get_current_user_id()==$item->author) ) {
						...
						if($result && wp_is_writable(VISION_PLUGIN_UPLOAD_DIR)) {
							...
							$file_main_css = 'main.css';
							$file_custom_css = 'custom.css';
							$file_root_path = VISION_PLUGIN_UPLOAD_DIR . '/' . $id . '/';
	...
	function admin_menu_page_item() {
		...
		if($page===VISION_PLUGIN_NAME . '_item') {
			...
			$globals = array(
				'plan' => VISION_PLUGIN_PLAN,
				'msg_pro_title' => __('Available only in Pro version', VISION_PLUGIN_NAME),
				'msg_edit_text' => __('Edit your text here', VISION_PLUGIN_NAME),
				'msg_custom_js_error' => __('Custom js code error', VISION_PLUGIN_NAME),
	...
	function ajax_item_update_status() {
		...
		if(check_ajax_referer(VISION_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if($result) {
				$data['id'] = $config->id;
				$data['msg'] = __('Item updated', VISION_PLUGIN_NAME);
			} else {
				$error = true;
				$data['msg'] = __('The operation failed, can\'t update item', VISION_PLUGIN_NAME);
			}
		} else {
			$error = true;
			$data['msg'] = __('The operation failed', VISION_PLUGIN_NAME);
		}
		...
		if(check_ajax_referer(VISION_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if(VISION_PLUGIN_PLAN == 'lite') {
				...
				if(!($rowcount == 0 || ($rowcount == 1 && $inputId))) {
					$flag = false;
					$error = true;
					$data['msg'] = __('The operation failed, you can work only with one item. To create more, buy the pro version.', VISION_PLUGIN_NAME);
				}
			}
			...
			if($flag) {
				...
				if($inputId) {
					...
					if($result) {
						...
						$data['msg'] = __('Item created', VISION_PLUGIN_NAME);
					} else {
						$error = true;
						$data['msg'] = __('The operation failed, can\'t create item', VISION_PLUGIN_NAME);
					}
				}
			}
	...
	function ajax_settings_update() {
		...
		if(check_ajax_referer(VISION_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if($result) {
				$data['msg'] = __('Settings updated', VISION_PLUGIN_NAME);
			} else {
				$error = true;
				$data['msg'] = __('The operation failed, can\'t update settings', VISION_PLUGIN_NAME);
			}
		}
	...
	function ajax_delete_data() {
		$error = true;
		$data = array();
		$data['msg'] = __('The operation failed, can\'t delete data', VISION_PLUGIN_NAME);
		...
		if(check_ajax_referer(VISION_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if($result) {
				$error = false;
				$data['msg'] = __('All data deleted', VISION_PLUGIN_NAME);
			}
		}
