...
class ImageLinks_Builder {
	...
	function run() {
		...
		define('IMAGELINKS_PLUGIN_UPLOAD_DIR', wp_normalize_path($upload_dir['basedir'] . '/' . IMAGELINKS_PLUGIN_NAME));
		define('IMAGELINKS_PLUGIN_UPLOAD_URL', $upload_dir['baseurl'] . '/' . IMAGELINKS_PLUGIN_NAME . '/');
		
		define('IMAGELINKS_PLUGIN_PLAN', 'lite');
		
		if(is_admin()) {
			$this->ajax_action_item_update = IMAGELINKS_PLUGIN_NAME . '_ajax_item_update';
			$this->ajax_action_item_update_status = IMAGELINKS_PLUGIN_NAME . '_ajax_item_update_status';
			$this->ajax_action_marker_template_save = IMAGELINKS_PLUGIN_NAME . '_ajax_marker_template_save';
			$this->ajax_action_marker_template_delete = IMAGELINKS_PLUGIN_NAME . '_ajax_marker_template_delete';
			$this->ajax_action_settings_update = IMAGELINKS_PLUGIN_NAME . '_ajax_settings_update';
			$this->ajax_action_settings_get = IMAGELINKS_PLUGIN_NAME . '_ajax_settings_get';
			$this->ajax_action_delete_data = IMAGELINKS_PLUGIN_NAME . '_ajax_delete_data';
			$this->ajax_action_modal = IMAGELINKS_PLUGIN_NAME . '_ajax_modal';
			
			load_plugin_textdomain(IMAGELINKS_PLUGIN_NAME, false, dirname(dirname(plugin_basename(__FILE__))) . '/languages/');
	...
	function shortcode($atts) {
		...
		if(!$id) {
			return '<p>' . __('Error: invalid imagelinks shortcode attributes', IMAGELINKS_PLUGIN_NAME) . '</p>';
		}
		...
		if($item) {
			...
			if(sizeof($this->shortcodes) == 1) {
				$plugin_url = plugin_dir_url(dirname(__FILE__));
				wp_enqueue_script(IMAGELINKS_PLUGIN_NAME . '_loader', $plugin_url . 'assets/js/loader.min.js', array('jquery'), IMAGELINKS_PLUGIN_VERSION, true);
				//wp_enqueue_script(IMAGELINKS_PLUGIN_NAME . '_es6promise', $plugin_url . 'assets/js/lib/html2canvas/es6-promise.min.js', array(), IMAGELINKS_PLUGIN_VERSION, true);
				//wp_enqueue_script(IMAGELINKS_PLUGIN_NAME . '_html2canvas', $plugin_url . 'assets/js/lib/html2canvas/html2canvas.min.js', array(), IMAGELINKS_PLUGIN_VERSION, true);
				...
				wp_localize_script(IMAGELINKS_PLUGIN_NAME . '_loader', IMAGELINKS_PLUGIN_NAME . '_globals', $globals);
			}
			...
			return '<p>' . __('Error: invalid imagelinks database record', IMAGELINKS_PLUGIN_NAME) . '</p>';
		}
	}
	...
	function admin_notices() {
		...
		if(!($page===IMAGELINKS_PLUGIN_NAME ||
			 $page===IMAGELINKS_PLUGIN_NAME . '_settings')) {
				 return;
		}
		...
		if(!file_exists(IMAGELINKS_PLUGIN_UPLOAD_DIR)) {
			...
			echo '<p>' . sprintf(__('The "%s" directory could not be created', IMAGELINKS_PLUGIN_NAME), '<b>' . IMAGELINKS_PLUGIN_NAME . '</b>') . '</p>';
			echo '<p>' . __('Please run the following commands in order to make the directory', IMAGELINKS_PLUGIN_NAME) . '<br>';
		...
		if(!wp_is_writable(IMAGELINKS_PLUGIN_UPLOAD_DIR)) {
			echo '<div class="notice notice-error is-dismissible">';
			echo '<p>' . sprintf(__('The "%s" directory is not writable, therefore the css and js files cannot be saved.', IMAGELINKS_PLUGIN_NAME), '<b>' . IMAGELINKS_PLUGIN_NAME . '</b>') . '</p>';
			echo '<p>' . __('Please run the following commands in order to make the directory', IMAGELINKS_PLUGIN_NAME) . '<br>';
			echo '<b>chmod 777 ' . IMAGELINKS_PLUGIN_UPLOAD_DIR . '</b></p>';
			echo '</div>';
			return;
	...
	function admin_menu() {
		...
		add_menu_page(__('ImageLinks', IMAGELINKS_PLUGIN_NAME), __('ImageLinks', IMAGELINKS_PLUGIN_NAME), 'manage_options', IMAGELINKS_PLUGIN_NAME, array( $this, 'admin_menu_page_items' ), 'dashicons-location');
		add_submenu_page(IMAGELINKS_PLUGIN_NAME, __('ImageLinks', IMAGELINKS_PLUGIN_NAME), __('All Items', IMAGELINKS_PLUGIN_NAME), 'manage_options', IMAGELINKS_PLUGIN_NAME, array( $this, 'admin_menu_page_items' ));
		add_submenu_page(IMAGELINKS_PLUGIN_NAME, __('ImageLinks', IMAGELINKS_PLUGIN_NAME), __('Add New', IMAGELINKS_PLUGIN_NAME), 'manage_options', IMAGELINKS_PLUGIN_NAME . '_item', array( $this, 'admin_menu_page_item' ));
		add_submenu_page(IMAGELINKS_PLUGIN_NAME, __('ImageLinks', IMAGELINKS_PLUGIN_NAME), __('Settings', IMAGELINKS_PLUGIN_NAME), 'manage_options', IMAGELINKS_PLUGIN_NAME . '_settings', array( $this, 'admin_menu_page_settings' ));
	}
	...
	function admin_menu_page_items() {
		...
		if($page===IMAGELINKS_PLUGIN_NAME) {
			...
			// styles
			wp_enqueue_style(IMAGELINKS_PLUGIN_NAME . '_admin_css', $plugin_url . 'assets/css/admin.min.css', array(), IMAGELINKS_PLUGIN_VERSION, 'all' );
			wp_enqueue_style(IMAGELINKS_PLUGIN_NAME . '_imagelinks_font_icons_css', $plugin_url . 'assets/css/imagelinks-font-icons.min.css', array(), IMAGELINKS_PLUGIN_VERSION, 'all' );
			
			// scripts
			wp_enqueue_script(IMAGELINKS_PLUGIN_NAME . '_ace', $plugin_url . 'assets/js/lib/ace/ace.js', array(), IMAGELINKS_PLUGIN_VERSION, false );
			wp_enqueue_script(IMAGELINKS_PLUGIN_NAME . '_admin_js', $plugin_url . 'assets/js/admin.min.js', array('jquery'), IMAGELINKS_PLUGIN_VERSION, false );
			
			// global settings to help ajax work
			$globals = array(
				'plan' => IMAGELINKS_PLUGIN_PLAN,
				'msg_pro_title' => __('Available only in the pro version', IMAGELINKS_PLUGIN_NAME),
				'upload_url' => $upload_dir['baseurl'],
				'ajax_url' => admin_url( 'admin-ajax.php' ),
				'ajax_nonce' => wp_create_nonce( IMAGELINKS_PLUGIN_NAME . '_ajax' ),
				'ajax_msg_error' => __('Uncaught Error', IMAGELINKS_PLUGIN_NAME) //Look at the console (F12 or Ctrl+Shift+I, Console tab) for more information
			);
			...
			if($action && $nonce && wp_verify_nonce($nonce, IMAGELINKS_PLUGIN_NAME)) {
				global $wpdb;
				$table = $wpdb->prefix . IMAGELINKS_PLUGIN_NAME;
				...
				if($action == 'duplicate') {
					...
					if($item && (current_user_can('administrator') || get_current_user_id()==$item->author) ) {
						$itemData = unserialize($item->data);
						$itemData->title = __('[Duplicate] ', IMAGELINKS_PLUGIN_NAME) . $itemData->title;
			...
			// set global settings
			wp_localize_script(IMAGELINKS_PLUGIN_NAME . '_admin_js', IMAGELINKS_PLUGIN_NAME . '_globals', $globals);
		}
	}
	...
	function admin_menu_page_item() {
		...
		if($page===IMAGELINKS_PLUGIN_NAME . '_item') {
			$plugin_url = plugin_dir_url(dirname(__FILE__));
			$upload_dir = wp_upload_dir();
			...
			wp_enqueue_style(IMAGELINKS_PLUGIN_NAME . '_admin_css', $plugin_url . 'assets/css/admin.min.css', array(), IMAGELINKS_PLUGIN_VERSION, 'all' );
			wp_enqueue_style(IMAGELINKS_PLUGIN_NAME . '_imagelinks_font_icons_css', $plugin_url . 'assets/css/imagelinks-font-icons.min.css', array(), IMAGELINKS_PLUGIN_VERSION, 'all' );
			wp_enqueue_style(IMAGELINKS_PLUGIN_NAME . '_fontawesome_font_icons_css', $plugin_url . 'assets/css/font-awesome.min.css', array(), IMAGELINKS_PLUGIN_VERSION, 'all' );
			wp_enqueue_style(IMAGELINKS_PLUGIN_NAME . '_imagelinks_effects_css', $plugin_url . 'assets/js/lib/imagelinks/imagelinks-effects.min.css', array(), IMAGELINKS_PLUGIN_VERSION, 'all' );
			...
			$globals = array(
				'plan' => IMAGELINKS_PLUGIN_PLAN,
				'msg_pro_title' => __('Available only in the pro version', IMAGELINKS_PLUGIN_NAME),
				'msg_edit_text' => __('Edit your text here', IMAGELINKS_PLUGIN_NAME),
				'msg_custom_js_error' => __('Custom js code error', IMAGELINKS_PLUGIN_NAME),
			...
			$settings_key = IMAGELINKS_PLUGIN_NAME . '_settings';
	...
	function admin_menu_page_settings() {
		...
		if($page===IMAGELINKS_PLUGIN_NAME . '_settings') {
			$plugin_url = plugin_dir_url(dirname(__FILE__));
	...
	function ajax_item_update_status() {
		...
		if(check_ajax_referer(IMAGELINKS_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			global $wpdb;
			...
			if($result) {
				$data['id'] = $config->id;
				$data['msg'] = __('Item updated', IMAGELINKS_PLUGIN_NAME);
			} else {
				$error = true;
				$data['msg'] = __('The operation failed, can\'t update the item', IMAGELINKS_PLUGIN_NAME);
			}
		} else {
			$error = true;
			$data['msg'] = __('The operation failed', IMAGELINKS_PLUGIN_NAME);
		}
		...
		if(check_ajax_referer(IMAGELINKS_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if(IMAGELINKS_PLUGIN_PLAN == 'lite') {
				...
				if(!($rowcount == 0 || ($rowcount == 1 && $inputId))) {
					$flag = false;
					$error = true;
					$data['msg'] = __('The operation failed, you can work only with one item. To create more, buy the pro version.', IMAGELINKS_PLUGIN_NAME);
				}
			}
			...
			if($flag) {
				if($inputId) {
					...
					if($result) {
						...
						$data['msg'] = __('Item created', IMAGELINKS_PLUGIN_NAME);
					} else {
						$error = true;
						$data['msg'] = __('The operation failed, can\'t create the item', IMAGELINKS_PLUGIN_NAME);
					}
				}
			}
	...
	function ajax_marker_template_save() {
		...
		if(check_ajax_referer(IMAGELINKS_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if($this->IsNullOrEmptyString($inputName)) {
				$error = true;
				$data['msg'] = __('The operation failed, can\'t create the template. Set the template name right', IMAGELINKS_PLUGIN_NAME);
			} else {
				//======================================
				// [filemanager] create an external file
				if(wp_is_writable(IMAGELINKS_PLUGIN_UPLOAD_DIR)) {
					...
					if(!file_exists($file_root_path . $file_json)) {
						...
						$data['msg'] = __('Template created', IMAGELINKS_PLUGIN_NAME);
					} else {
						$error = true;
						$data['msg'] = __('The operation failed, can\'t create the template. The template already exists with the same name', IMAGELINKS_PLUGIN_NAME);
					}
				} else {
					$error = true;
					$data['msg'] = __('The operation failed, can\'t create the template. The template folder is unwritable', IMAGELINKS_PLUGIN_NAME);
				}
				//======================================
			}
		} else {
	...
	function ajax_marker_template_delete() {
		...
		if(check_ajax_referer(IMAGELINKS_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if(wp_is_writable(IMAGELINKS_PLUGIN_UPLOAD_DIR)) {
				...
				if(file_exists($file_root_path . $file_json)) {
					wp_delete_file($file_root_path . $file_json);
					$data['msg'] = __('Template deleted', IMAGELINKS_PLUGIN_NAME);
				} else {
					$error = true;
					$data['msg'] = __('The operation failed, can\'t delete the template. The template does not exists', IMAGELINKS_PLUGIN_NAME);
				}
	...
	function ajax_settings_update() {
		...
		if(check_ajax_referer(IMAGELINKS_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if($result) {
				$data['msg'] = __('Settings updated', IMAGELINKS_PLUGIN_NAME);
			} else {
				$error = true;
				$data['msg'] = __('The operation failed, can\'t update settings', IMAGELINKS_PLUGIN_NAME);
			}
		}
		...
		if(check_ajax_referer(IMAGELINKS_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			switch($type) {
				...
				case 'import-items' : {
					...
					foreach($wpdb->get_results('SELECT ID, post_title, post_modified FROM ' . $table . ' WHERE post_type="imgl_item"') as $key=>$item) {
				...
				case 'import-items-count' : {
					global $wpdb;
					$table = $wpdb->prefix . 'posts';
					$count = (int)$wpdb->get_var('SELECT COUNT(*) FROM ' . $table . ' WHERE post_type="imgl_item"');
	...
	function ajax_delete_data() {
		$error = true;
		$data = array();
		$data['msg'] = __('The operation failed, can\'t delete data', IMAGELINKS_PLUGIN_NAME);
		...
		if(check_ajax_referer(IMAGELINKS_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if($result) {
				$error = false;
				$data['msg'] = __('All data deleted', IMAGELINKS_PLUGIN_NAME);
			}
		}
	...
	function ajax_import_data() {
		$error = true;
		$data = array();
		$data['msg'] = __('The operation failed, can\'t import data', IMAGELINKS_PLUGIN_NAME);
		...
		if(check_ajax_referer(IMAGELINKS_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			$data['list'] = array();
			foreach($wpdb->get_results('SELECT ID, post_title FROM ' . $tablePosts . ' WHERE post_type="imgl_item"') as $key=>$item) {
			...
			$error = false;
			$data['msg'] = __('All data imported', IMAGELINKS_PLUGIN_NAME);
		}
