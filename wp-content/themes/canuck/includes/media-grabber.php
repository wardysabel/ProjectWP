 ...
 */
/**
 * Wrapper function for the Hybrid_Media_Grabber class.  Returns the original media for the found media.
 *
 * @since  1.6.0
 * @access public
 * @param  array
 ...
 * added by KA
 */
function canuck_media_grabber_audio( $args = array() ) {
	$media = new Canuck_Media_Grabber( $args );
 ...
 * added by KA
 */
function canuck_media_grabber_video( $args = array() ) {
	$media = new Canuck_Media_Grabber( $args );
	return $media->original_media;
}
/**
 ...
 * added by KA
 */
function canuck_media_grabber_gallery( $args = array() ) {
	$media = new Canuck_Media_Grabber( $args );
	return $media->original_media;
}
/**
 ...
 * @return string
 */
function canuck_media_grabber( $args = array() ) {
...
class Canuck_Media_Grabber {
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'canuck_media_grabber_embed_shortcode_media', array( $wp_embed, 'run_shortcode' ) );
		add_filter( 'canuck_media_grabber_autoembed_media',       array( $wp_embed, 'autoembed'     ) );
		...
		$this->args    = apply_filters( 'canuck_media_grabber_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_media() {
		return apply_filters( 'canuck_media_grabber_media', $this->media, $this );
	}
		...
		if ( ! empty( $matches ) ) {
			...
			foreach ( $matches as $shortcode ) {
				...
				if ( in_array( $shortcode[2], array( 'playlist', 'embed', $this->type ) ) ) {
					call_user_func( array( $this, "do_{$shortcode[2]}_shortcode_media" ), $shortcode ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					break;
				}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = apply_filters(
			'canuck_media_grabber_embed_shortcode_media',
			$this->original_media
		);
	}
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'canuck_media_grabber_autoembed_media', $value[0] ) );

				if ( $embed ) {

					// If we're given a shortcode, roll with it.
					if ( preg_match( "/\[{$this->type}\s/", $embed ) ) { // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	public function filter_dimensions( $html ) {
		...
		list( $width, $height ) = apply_filters(
			'canuck_media_grabber_dimensions',
