 	...
 	if (!function_exists('iamaze_custom_styles')) { 
		function iamaze_custom_styles() {
			...
			if ( function_exists( 'rwmb_meta' ) ) {
				$custom_page_color = rwmb_meta('iamaze_page_color', '');			
			...
			*/
			...
			/*
			if ( isset( $tx_title_font['font-weight'] ) ) {
				$tx_title_style .= 'font-weight: '.$tx_title_font['font-weight'].'; ';
			}				
			*/
			...
			echo '.ibanner,.da-dots span.da-dots-current,.tx-cta a.cta-button, .utilitybar, #footer-widgets ins {background-color: '.$primary_color.';}';
			...
			echo '.entry-header h1.entry-title a:hover,.entry-header > .entry-meta a:hover, #footer-widgets .woocommerce ul .star-rating span:before {color: '.$primary_color.';}';
			...
			echo 'a.txoibutton, a.txoibutton:visited, .header-icons.woocart .cartdrop.widget_shopping_cart.nx-animatee { border-color:'.$primary_color.'; }';
